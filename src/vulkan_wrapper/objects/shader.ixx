...
public:
	...
	[[nodiscard]] shader_module(VkDevice device, const std::string_view path) : shader_module(
	...
	[[nodiscard]] shader_stage_bundle get_stage_bundle(
				VkShaderStageFlagBits stage,
				const std::string_view entry = "main",
				const VkSpecializationInfo* specializationInfo = nullptr
			) const noexcept {
		return {
			get_create_info(stage, entry, specializationInfo),
			get_binary()
		};
	}
