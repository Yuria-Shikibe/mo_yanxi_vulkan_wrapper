...
export
template <typename ImageProv>
struct combined_image_type{
protected:
	ImageProv image{};
	image_view image_view{};

public:
	[[nodiscard]] combined_image_type() = default;

	[[nodiscard]] combined_image_type(ImageProv&& image, const VkImageViewCreateInfo& image_view_create_info)
		: image(std::move(image)),
		  image_view(this->image.get_device(), [](VkImage img, VkImageViewCreateInfo info) static{
			  info.image = img;
			  return info;
		  }(static_cast<VkImage>(this->image), image_view_create_info)){
	}
...
export
using combined_image = combined_image_type<image>;
...
export constexpr std::uint32_t get_recommended_mip_level(const VkExtent2D extent,
                                                         const std::uint32_t ceil = std::numeric_limits<
...
public:
	...
	void write(
		...
		for(const texture_buffer_write& arg : args){
			cmd::copy_buffer_to_image(command_buffer, arg.buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
